...
class DataLoader:
        ...
        for tag_file in file_collections:
            with open(tag_file, 'r') as f:
                ...
                for row in rows:
                    ...
                    if hair_color not in self.hair_color_dict:
                        self.hair_color_dict.append(hair_color)
                    if eye_color not in self.eye_color_dict:
                        self.eye_color_dict.append(eye_color)

                    tags.append([self.hair_color_dict.index(hair_color), self.eye_color_dict.index(eye_color)])
    ...
    def gen_one_hot(self, inputs):
        ...
        one_hot_vecs = []
        for tag in inputs:
            hair_vec = np.zeros(len(self.hair_color_dict))
            hair_vec[tag[0]] = 1
            eye_vec = np.zeros(len(self.eye_color_dict))
            eye_vec[tag[1]] = 1
    ...
    def create_testing_tags(self):
        ...
        for file in testing_tag_file:
            ...
            for i in range(self.sample_img_size):
                hair_tag = random.sample(self.hair_color_dict, 1)
                eye_tag = random.sample(self.eye_color_dict, 1)
                for _ in range(self.sample_img_size):
                    string = str(counter) + ',' + hair_tag[0] + ' hair ' + eye_tag[0] + ' eyes\n'
                    f.write(string)
                    counter += 1
