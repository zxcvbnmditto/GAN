...
class ACGAN():
    ...
    def discriminator(self, x, reuse=False):
        ...
        with tf.variable_scope('discriminator') as scope:
            ...
            # eval the tags of the image
            hair_logit = tf.layers.dense(flatten, units=13, trainable=True)

            eye_logit = tf.layers.dense(flatten, units=13, trainable=True)

        return real_fake_logit, hair_logit, eye_logit
    ...
    def build(self):
        ...
        self.c_real_label , self.c_hair_tag, self.c_eye_tag = self.discriminator(self.correct_imgs, reuse=False)
        self.f_real_label, self.f_hair_tag, self.f_eye_tag = self.discriminator(self.fake_imgs, reuse=True)

        self.temp1 = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.c_real_label, labels=tf.ones_like(self.c_real_label))
        self.temp2 = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.f_real_label, labels=tf.zeros_like(self.f_real_label))
        self.temp3 = tf.nn.softmax_cross_entropy_with_logits(logits=self.c_hair_tag, labels=self.correct_tags[:, :13])
        self.temp4 = tf.nn.softmax_cross_entropy_with_logits(logits=self.f_hair_tag, labels=self.correct_tags[:, :13])
        self.temp5 = tf.nn.softmax_cross_entropy_with_logits(logits=self.c_eye_tag, labels=self.correct_tags[:, 13:])
        self.temp6 = tf.nn.softmax_cross_entropy_with_logits(logits=self.f_eye_tag, labels=self.correct_tags[:, 13:])

        self.loss_source = tf.reduce_mean(self.temp1) + tf.reduce_mean(self.temp2)
        self.loss_condition = tf.reduce_mean(tf.reduce_sum(self.temp3)) + tf.reduce_mean(tf.reduce_sum(self.temp4)) + tf.reduce_mean(tf.reduce_sum(self.temp5)) + tf.reduce_mean(tf.reduce_sum(self.temp6))


        # ACGAN
        self.d_loss = -(self.loss_source + self.loss_condition)
        self.g_loss = -(self.loss_condition - self.loss_source)
